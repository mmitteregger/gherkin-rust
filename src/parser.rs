...
impl<B: Builder> Parser<B> {
    ...
    pub fn parse_str_with_matcher<S: AsRef<str>, TM>(
        &mut self,
        source: S,
        token_match: &mut TM,
    ) -> Result<B::BuilderResult>
    where
        TM: TokenMatch,
    {
    ...
    pub fn parse_reader<R>(&mut self, source: R) -> Result<B::BuilderResult>
    where
        R: Read,
    {
    ...
    pub fn parse_reader_with_matcher<R, TM>(
        &mut self,
        source: R,
        token_match: &mut TM,
    ) -> Result<B::BuilderResult>
    ...
    pub fn parse_scanner<TS>(&mut self, token_scan: &mut TS) -> Result<B::BuilderResult>
    where
        TS: TokenScan,
    {
    ...
    pub fn parse_scanner_with_matcher<TS, TM>(
        &mut self,
        token_scan: &mut TS,
        token_match: &mut TM,
    ) -> Result<B::BuilderResult>
    ...
    fn lookahead_0(&mut self, context: &mut ParserContext, current_token: &Token) -> bool {
        ...
        loop {
            ...
            if false
                || self
                    .match_examples_line(context, &mut *token.borrow_mut())
                    .unwrap_or(false)
            {
                found_match = true;
                break;
            ...
            if true && !self
                .match_empty(context, &mut *token.borrow_mut())
                .unwrap_or(true)
                && !self
                    .match_comment(context, &mut *token.borrow_mut())
                    .unwrap_or(true)
                && !self
                    .match_tag_line(context, &mut *token.borrow_mut())
                    .unwrap_or(true)
            {
                break;
            }
