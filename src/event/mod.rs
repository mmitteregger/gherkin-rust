...
pub fn generate<D: Into<String>, U: AsRef<str>>(data: D, uri: U)
...
pub fn generate_with_language<D: Into<String>, U: AsRef<str>, L: Into<String>>(
        data: D, uri: U, language: L) -> Result<Vec<Box<CucumberEvent>>> {
...
fn generate_with_matcher<D: Into<String>, U: AsRef<str>, DP: GherkinDialectProvide>(
        data: D, uri: U, mut token_matcher: TokenMatcher<DP>)
        -> Result<Vec<Box<CucumberEvent>>> {
    ...
    let gherkin_document =
        parser.parse_str_with_matcher(&data, &mut token_matcher)?;
    ...
    events.push(Box::new(GherkinDocumentEvent::new(uri.to_owned(), gherkin_document)));
